...
class AuthenticationHelper {
     ...
     * @return string
     * @throws \MangoPay\Types\Exceptions\Exception If MangoPay_ClientId or MangoPay_ClientPassword is not defined
     */
    public function GetHttpHeaderBasicKey() {
        if (is_null($this->_root->Config->ClientId) || strlen($this->_root->Config->ClientId) == 0)
            throw new Exceptions\Exception ('MangoPayApi.Config.ClientId is not set.');

        if (is_null($this->_root->Config->ClientPassword) || strlen($this->_root->Config->ClientPassword) == 0)
            throw new Exceptions\Exception ('MangoPayApi.Config.ClientPassword is not set.');
     ...
     *
     * @return string Value for HTTP header with authentication string
     * @throws \MangoPay\Types\Exceptions\Exception If OAuth token is not created (or is invalid) for strong authentication.
     */
