...
class RestTool {
     ...
     * @var \MangoPay\Enums\RequestType
     */
     ...
     * @param \MangoPay\Enums\RequestType $requestType Type of request
    ...
    private function RunRequest() {
        ...
        if ($result === false && curl_errno($this->_curlHandle) != 0)
            throw new \MangoPay\Types\Exceptions\Exception('cURL error: ' . curl_error($this->_curlHandle));
    ...
    private function BuildRequest($urlMethod, $pagination, $additionalUrlParams = null) {
        ...
        if ($this->_curlHandle === false)
            throw new \MangoPay\Types\Exceptions\Exception('Cannot initialize cURL session');
    ...
    private function CheckResponseCode($response){
        ...
        if ($this->_responseCode != 200) {
            ...
            if (isset($response) && is_object($response) && isset($response->Message)) {
                ...
                $error = new \MangoPay\Types\Error();
                $error->Message = $response->Message;
                $error->Errors = property_exists($response, 'Errors')
                        ? $response->Errors
                        : property_exists($response, 'errors') ? $response->errors : null;
                throw new \MangoPay\Types\Exceptions\ResponseException($this->_requestUrl, $this->_responseCode, $error);

            } else
                throw new \MangoPay\Types\Exceptions\ResponseException($this->_requestUrl, $this->_responseCode);
        }
    }
}
