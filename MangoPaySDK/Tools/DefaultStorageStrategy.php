...
class DefaultStorageStrategy implements IStorageStrategy {
    ...
    public function Store($token) {
        ...
        if (!is_writable($this->GetPathToTemporaryFolder()))
            throw new \MangoPay\Types\Exceptions\Exception('Cannot create or write to file ' . $this->GetPathToTemporaryFolder());

        $serialized = serialize($token);
        $result = file_put_contents($this->GetPathToFile(), $this->_prefixContent . $serialized, LOCK_EX);
        if ($result === false)
            throw new \MangoPay\Types\Exceptions\Exception('Cannot put token to file');
    }
    ...
    private function GetPathToTemporaryFolder() {
        ...
        if (is_null($this->_config->TemporaryFolder))
            throw new \MangoPay\Types\Exceptions\Exception('Path to temporary folder is not defined');
